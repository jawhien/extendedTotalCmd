...
from SCons.Script import BoolVariable, Builder, Copy, Environment, DefaultEnvironment, Variables
...
env = DefaultEnvironment(variables=vars, ENV=os.environ, tools=['gettexttool', mdTool])
...
addonFile = env.File("${addon_buildFileName}-${addon_version}.nvda-addon")
...
def createAddonHelp(dir):
	...
	if os.path.isfile("LICENSE.txt"):
		...
		env.Depends(addon, licenseTarget)
